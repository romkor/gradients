...
The system SHALL read the current user session in the TanStack Router root route loader using `auth.api.getSession`. The resulting `user` object (or `null`) SHALL be passed into TanStack Router context so all child routes and components can access it without additional fetches.
